...
function doseText(drug: DrugDef, wtKg: number, concMgPerMl: number) {
  ...
  const unit = drug.unitPerKg // 'mg' or 'mcg'
  const toMg = unit === 'mcg' ? 1 / 1000 : 1 // convert to mg for mL calc
  ...
  if (loPerKg === hiPerKg) {
    return {
      headline: `${loPerKg} ${unit}/kg`,
      mgStr: `${round(loMg, 2)} mg`,
      mlStr: `${round(loMl, 2)} mL`,
    }
  }
  return {
    headline: `${loPerKg}–${hiPerKg} ${unit}/kg`,
    mgStr: `${round(loMg, 2)}–${round(hiMg, 2)} mg`,
    mlStr: `${round(loMl, 2)}–${round(hiMl, 2)} mL`,
  }
}
...
export function DrugDoseCards() {
  ...
  React.useEffect(() => {
    localStorage.setItem('patient.weight.kg', String(wtKg))
  }, [wtKg])
          ...
          return (
            <article key={d.key} className="rounded-2xl border bg-white p-3 shadow-sm">
              ...
              <div className="mt-2 text-sm">
                ...
                <div className="mt-1">
                  <span className="font-medium">Dose:</span> {txt.mgStr} &nbsp;(
                  <span className="font-medium">≈</span> {txt.mlStr})
                </div>
              ...
              <div className="mt-2 flex gap-2">
                <button
                  ...
                  onClick={() => {
                    const line = `${d.label}: ${txt.mgStr} (≈ ${txt.mlStr} @ ${c} mg/mL)`
