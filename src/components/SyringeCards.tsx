    ...
    lines.push(
      <div key="f" className="text-sm">
        {s.fixed.map((f, idx) => {
          ...
          const conc: number | undefined = typeof stock.mgPerMl === 'number' ? stock.mgPerMl : undefined
          const mg = conc ? conc * f.volumeMl : null
          usedVol += f.volumeMl
          return (
...
export default function SyringeCards() {
  ...
  return (
    // Remove outer padding; cancel main's padding with negative margins, then re-add inside.
    <section className="mx-auto max-w-none lg:max-w-3xl pb-6">
      <div className="-mx-2 sm:-mx-3 md:-mx-4 lg:-mx-6 px-2 sm:px-3 md:px-4 lg:px-6">
