...
export type TabKey = 'home' | 'syringes' | 'timer' | 'doses' | 'ett' | 'la' | 'fluids' | 'mac'
...
export default function TabsBar({ current, setCurrent }: { current: TabKey, setCurrent: (k: TabKey)=>void }) {
  const Item = ({ k, label }: { k: TabKey, label: string }) => (
    <button
      ...
      className={`px-3 py-2 text-sm whitespace-nowrap ${current===k? 'font-semibold underline' : 'opacity-80'}`}
